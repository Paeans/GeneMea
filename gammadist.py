...
fwlist       = [x/10.0 for x in range(1, 10)]
#vwlist       = [0.1, 100]
...
def calcmotifscore(binscorematrix):
    ...
    for gdist in gammalist:
        ...
        for k in range(listsize):
            ...
            for bindex in range(binsize - 1):
                # f1 score normalized by the max value of each column
                tmp += gdist[bindex] * binscorematrix[bindex][k] / maxlist[bindex]
                # f1 score non normalized
                # tmp += gdist[bindex] * binscorematrix[bindex][k]
                #
    ...
    for i in range(1, binsize - 1):
        ...
        # normalize f2 score
        maxf1 = max(binscorematrix[i-1]) - min(binscorematrix[i-1])
        if maxf1 == 0:
            maxf1 = 1
        for j in range(listsize):
            binscorematrix[i-1][j] -= binscorematrix[i][j]
            binscorematrix[i-1][j] = binscorematrix[i-1][j]/maxf1
    for k in range(listsize):
        vscore = 0
        for i in range(binsize - 2):
            if(binscorematrix[i][k] > -700):
                vscore += 1/(1 + math.exp(-binscorematrix[i][k]))
        ...
        for fw in fwlist:
            ...
            for vw in vwlist:
                ...
                result.append((maxindex, float("{0:.5f}".format(maxfvalue)), \
                                ...
                                float("{0:.5f}".format(fw * maxfvalue + vw * 100 * (maxvvalue - 0.4)))))
    return result
...
if __name__ == "__main__":
    ...
    result = [[] for i in range(len(gammalist)) * len(fwlist)]
