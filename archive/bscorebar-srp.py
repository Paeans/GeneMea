...
if __name__ == "__main__":
    ...
    for mname in ['V$E2F_03', 'V$OCT1_07']:
        ...
        #print scorematrix
        tmpmatrix = calcmotifscore(scorematrix)
        resultmatrix.append(tmpmatrix)
    
    ax1 = plt.subplot(121)
    plt.title('a) V$E2F_03')
    plt.xlabel('bins index')
    plt.ylabel('Sequence-Specific Binding Score')
    plt.bar(np.arange(40), resultmatrix[0], 1, color='y')
    plt.yticks(np.arange(0, 0.6, 0.1), np.arange(0.5, 1.1, 0.1))
    
    ax2 = plt.subplot(122)
    plt.title('b) V$OCT1_07')
    plt.xlabel('bins index')    
    plt.bar(np.arange(40), resultmatrix[1], 1, color='y')
    plt.yticks(np.arange(0, 0.6, 0.1), np.arange(0.5, 1.1, 0.1))
