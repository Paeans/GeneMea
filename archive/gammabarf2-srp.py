...
def calcmotifscore(binscorematrix):
    ...
    print len(binscorematrix[0])
    for i in range(41):
        result.append(binscorematrix[i][136300])
    return result
...
if __name__ == "__main__":
    ...
    for mname in ['V$E2F_03', 'V$OCT1_07']:
        
        motifpwm = getmotiflist(motifilename, prefix = mname)[0]
        ...
        tmpdata = calcmotifscore(scorematrix)
        data = []
        for i in range(len(tmpdata) - 1):
            #tmp = 1/(1 + math.exp((tmpdata[i+1] - tmpdata[i])/(10000))) - 0.4
            data.append(tmpdata[i] - tmpdata[i+1])
            #data.append(tmp)
        negative_data = [] 
        positive_data = [] 
        for d in data:
        ...
        index += 1
    plt.figlegend((p1[0], p2[0]), ('positive score', 'negative score'), loc='lower center', bbox_to_anchor=(0.5, -0.04), ncol = 3)
